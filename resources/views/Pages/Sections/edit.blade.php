                        ...
                        <div class="form-check">
                            @if ($list_Sections->Status === 1)
                                <input type="checkbox" checked
                                       class="form-check-input" name="Status" id="exampleCheck1">
                            @else
                    ...
                    <div class="col">
                        ...
                        <select multiple name="teacher_id[]" class="form-control" id="exampleFormControlSelect2">
                                @foreach($teachers as $teacher)
                                    @php $check = []; @endphp
                                    @foreach ($list_Sections->teachers as $key => $teacherD  )
                                    @php
                                        $check[] = $teacherD->id ; //[1,2,3]
                                    @endphp
                                    @endforeach
                                    <option value="{{$teacher->id}}" {{ in_array($teacher->id, $check) ? 'selected' : '' }}>{{$teacher->Name}}</option>
                                @endforeach
                        </select>
                    </div>
